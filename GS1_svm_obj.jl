...
function main()
    for dataset_name in dataset_names
        ...
        for i in 1:maxIter
            grad_q = g_q(q)
            alphahat, localq = GSq_project(q, grad_q, L2 / 4, lower, upper, y_train)
            alpha[:] = Float64.(alphahat[:])
            q[:] = localq[:]
            if mod(i, trackIter) == 0
                @printf("Iteration %d, Function = %f\n", i, f(alpha))
            ...
            # now compute the predictions
            X_support_gsq, y_support_gsq = get_support_vectors(X_train, y_train, alpha; threshold=1e-6)
            b = compute_bias(X_support, y_support, alpha)
            train_y_predicted_gsq = svm_predict_kernel(X_support_gsq, y_support_gsq, alpha[support_vectors_GSQ], b, X_train)
